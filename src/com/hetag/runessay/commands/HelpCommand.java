...
public class HelpCommand extends RSCommand {
	...
	public HelpCommand() {
		super("help", "/rs help <Page/Topic>", config.getString(path + "Description"), new String[] { "help", "h" });
	...
	public void execute(CommandSender sender, List<String> args) {
		...
		if (args.size() == 0) {
			...
			for (RSCommand command : instances.values()) {
				if ((!command.getName().equalsIgnoreCase("help")) && (sender.hasPermission("runessay.command." + command.getName()))) {
		...
		if (isNumeric(arg)) {
			...
			((RSCommand) instances.get(arg)).help(sender, true);
		} else {
